            ...
            <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-12">
                ...
                <div className="space-y-4">
                    <div className="flex items-center gap-2 text-white font-black text-xl uppercase tracking-tighter">
                        ...
                        <span>Texas<span className="text-blue-500">TotalLoss</span></span>
                    </div>
                    <p className="text-xs text-gray-500 leading-relaxed">
                        {dict.footer.disclaimer_text}
                    </p>
                    <div className="pt-4 flex gap-4">
                        <FacebookIcon size={20} className="hover:text-blue-500 cursor-pointer transition" />
                        <TwitterIcon size={20} className="hover:text-blue-400 cursor-pointer transition" />
                        <InstagramIcon size={20} className="hover:text-pink-500 cursor-pointer transition" />
                    </div>
                </div>
                ...
                {/* Column 3: Resources */}
                <div>
                    <h3 className="text-white font-bold uppercase tracking-wider mb-6 text-sm">Resources</h3>
                    <ul className="space-y-3 text-sm">
                ...
                {/* Column 4: Legal */}
                <div>
                    <h3 className="text-white font-bold uppercase tracking-wider mb-6 text-sm">Legal</h3>
                    <ul className="space-y-3 text-sm">
                        <li><Link href="/privacy" className="hover:text-white transition">Privacy Policy</Link></li>
                        <li><Link href="/terms" className="hover:text-white transition">Terms of Service</Link></li>
                        <li><Link href="/sitemap" className="hover:text-white transition">Sitemap</Link></li>
                        <li><Link href="/disclosure" className="text-yellow-500 hover:text-yellow-400 font-bold transition">Full Disclosure</Link></li>
                    </ul>
                </div>
            </div>

            <div className="max-w-6xl mx-auto mt-12 pt-8 border-t border-gray-800 text-center md:text-left flex flex-col md:flex-row justify-between items-center text-xs text-gray-600">
                <p>&copy; {year} Texas Total Loss Claim Help. {dict.footer.rights}</p>
                <div className="mt-4 md:mt-0">
                    <p>{dict.footer.disclaimer_title}</p>
                </div>
            </div>
        </footer>
    );
